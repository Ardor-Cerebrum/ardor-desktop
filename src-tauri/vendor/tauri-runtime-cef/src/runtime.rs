...
fn select_remote_debugging_port() -> i32 {
  ...
  let span = REMOTE_DEBUGGING_MAX_PORT - REMOTE_DEBUGGING_MIN_PORT + 1;
  let seed = std::time::SystemTime::now()
    .duration_since(std::time::UNIX_EPOCH)
    .map(|duration| duration.as_nanos() as u64)
    .unwrap_or_default()
    ^ std::process::id() as u64;
  let start = REMOTE_DEBUGGING_MIN_PORT + (seed % span as u64) as u16;

  for offset in 0..span {
    let port = REMOTE_DEBUGGING_MIN_PORT + ((start - REMOTE_DEBUGGING_MIN_PORT + offset) % span);
  ...
  TcpListener::bind((Ipv4Addr::LOCALHOST, 0))
    ...
    .map(|address| address.port() as i32)
    .unwrap_or_default()
}
...
/// Installs the sandbox context supplied by the CEF M138+ bootstrap process.
///
/// The pointer is process-local, owned by `bootstrap.exe`, and remains valid for
/// the duration of `RunWinMain`. It must be installed before any CEF API call.
#[cfg(windows)]
pub fn install_windows_sandbox_info(
  sandbox_info: *mut u8,
) -> std::result::Result<(), &'static str> {
  if !cfg!(feature = "sandbox") || sandbox_info.is_null() {
    return Err(
      "CEF Windows sandbox context is unavailable; launch the packaged bootstrap executable",
    );
  }
  ...
  WINDOWS_SANDBOX_INFO
    ...
    .map(|_| ())
    .or_else(|current| {
      if current == sandbox_info {
        Ok(())
      } else {
        Err("CEF Windows sandbox context changed after initialization")
      }
    })
}

#[cfg(windows)]
pub fn windows_sandbox_active() -> bool {
  cfg!(feature = "sandbox") && !WINDOWS_SANDBOX_INFO.load(Ordering::Acquire).is_null()
}

#[cfg(windows)]
fn windows_sandbox_info() -> *mut u8 {
  let sandbox_info = WINDOWS_SANDBOX_INFO.load(Ordering::Acquire);
  assert!(
    !sandbox_info.is_null(),
    "CEF on Windows must be launched through the M138+ bootstrap executable"
  );
  sandbox_info
}
...
use crate::{deny_web_permissions, external_message_pump::CefExternalPump};
#[cfg(target_os = "macos")]
...
impl<T: UserEvent> WinitCefApp<T> {
  ...
  pub(crate) fn close_window(&mut self, window_id: WindowId, event_loop: &dyn ActiveEventLoop) {
    // Programmatic close/destroy drops the winit window immediately. Its later
    // OS `Destroyed` event can no longer be mapped after we remove the window
    // from runtime state, so notify Tauri first. Without this, AppManager keeps
    // stale window labels forever and short-lived overlay portals accumulate.
    if !self.state.exiting {
...
impl<T: UserEvent> CefRuntime<T> {
  fn init(
    ...
    let cef_log_file = std::env::var_os("ARDOR_CEF_LOG_FILE")
      .map(|value| value.to_string_lossy().into_owned())
      .unwrap_or_default();
    let settings = cef::Settings {
      no_sandbox: !cfg!(feature = "sandbox") as i32,
      cache_path: cache_path.to_string_lossy().to_string().as_str().into(),
      log_file: cef_log_file.as_str().into(),
      log_severity: if cef_log_file.is_empty() {
        cef::LogSeverity::DEFAULT
      } else {
        cef::LogSeverity::INFO
      },
    ...
    browser_client::trace_devtools(format!(
      "cef_initialize.settings cache_path={:?} cef_log_file={:?} log_severity={:?} external_message_pump={} windowless_rendering_enabled={} remote_debugging_port={}",
      cache_path,
      cef_log_file,
