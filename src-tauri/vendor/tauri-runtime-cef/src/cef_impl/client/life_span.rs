...
fn check_and_reload_if_blank(browser: cef::Browser, initial_url: String) {
  if initial_url == "about:blank" || disable_blank_reload_guard() {
    return;
  }
...
wrap_life_span_handler! {
  ...
  impl LifeSpanHandler {
    ...
    fn on_before_dev_tools_popup(
      ...
      let window_info_present = window_info.is_some();
      let client_present = client.is_some();
      let use_default_window_present = use_default_window.is_some();
      trace_devtools(format!(
        "on_before_dev_tools_popup.enter browser_id={} browser_present={} window_info_present={} client_present={} use_default_window_present={}",
        browser_id,
        browser_present,
        window_info_present,
        client_present,
        use_default_window_present
      ));
      ...
      trace_devtools(format!("on_before_dev_tools_popup.exit browser_id={browser_id}"));
    }
