...
fn csp_inject_initialization_scripts_hashes(
  ...
  let has_script_src_elem = csp_map.contains_key("script-src-elem");
  for directive in ["script-src", "script-src-elem"] {
    if directive == "script-src-elem" && !has_script_src_elem {
      continue;
    }
    ...
    sources.extend(script_hashes.clone());
  }
...
fn inject_scripts_into_html_body(
  ...
  if let Ok(meta_elements) = document.select("meta[http-equiv]") {
    for meta in meta_elements {
      let Some(element) = meta.as_node().as_element() else {
        continue;
      };
      ...
      if let Some(existing_csp) = attributes.get("content").map(str::to_owned) {
        attributes.insert(
          "content",
          csp_inject_initialization_scripts_hashes(existing_csp, initialization_scripts),
        );
      }
    }
  }
...
mod tests {
  ...
  fn init_script() -> CefInitScript {
    CefInitScript {
      script: "window.__TAURI_TEST__ = true;".to_string(),
      hash: "'sha256-tauri-test'".to_string(),
      for_main_frame_only: true,
    }
  }

  #[test]
  fn adds_hashes_to_script_and_element_directives() {
    let updated = csp_inject_initialization_scripts_hashes(
      "default-src 'self'; script-src 'self'; script-src-elem 'self' https://example.com"
        .to_string(),
      &[init_script()],
    );

    assert_eq!(updated.matches("'sha256-tauri-test'").count(), 2);
  }

  #[test]
  fn updates_meta_csp_before_injecting_scripts() {
    let body = br#"<!doctype html><html><head><meta http-equiv="Content-Security-Policy" content="default-src 'self'; script-src 'self'; script-src-elem 'self'"></head><body></body></html>"#;
    let injected = inject_scripts_into_html_body(body, &[init_script()]).unwrap();
    let document = parse_html(String::from_utf8(injected).unwrap());
    let meta = document
      .select_first("meta[http-equiv]")
      .expect("CSP meta element must remain present");
    let element = meta
      .as_node()
      .as_element()
      .expect("selected node must be an element");
    let attributes = element.attributes.borrow();
    let content = attributes
      .get("content")
      .expect("CSP meta element must keep its content");

    assert_eq!(content.matches("'sha256-tauri-test'").count(), 2);
    assert!(String::from_utf8_lossy(&serialize_node(&document)).contains("window.__TAURI_TEST__"));
  }
}
