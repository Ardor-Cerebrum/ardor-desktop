...
use crate::offscreen::{BrowserAudioState, OffscreenFrame, OffscreenSurface};
...
impl Webview {
  ...
  pub fn offscreen_surface(&self) -> Option<OffscreenSurface> {
    self.offscreen_surface.clone()
  }

  pub fn audio_state(&self) -> Option<BrowserAudioState> {
    self.audio_state.clone()
  }

  pub fn latest_offscreen_frame(&self) -> Option<OffscreenFrame> {
    self
      .offscreen_surface
      .as_ref()
      .and_then(OffscreenSurface::latest_frame)
  }

  pub fn send_offscreen_mouse_move(&self, event: cef::MouseEvent, mouse_leave: bool) {
    if let Some(host) = self.browser.host() {
      host.send_mouse_move_event(Some(&event), mouse_leave as i32);
    }
  }

  pub fn send_offscreen_mouse_click(
    &self,
  ...
  pub fn send_offscreen_mouse_wheel(&self, event: cef::MouseEvent, delta_x: i32, delta_y: i32) {
    if let Some(host) = self.browser.host() {
      host.send_mouse_wheel_event(Some(&event), delta_x, delta_y);
    }
  }

  pub fn send_offscreen_key_event(&self, event: cef::KeyEvent) {
    if let Some(host) = self.browser.host() {
...
impl AppWebview {
  pub(crate) fn set_bounds(&mut self, parent_size: PhysicalSize<u32>, scale: f64, bounds: Rect) {
    let is_offscreen = self.offscreen_surface.is_some();
    if let Some(surface) = &self.offscreen_surface {
      surface.set_bounds(bounds, scale);
    }
...
impl<T: UserEvent> WinitCefApp<T> {
  ...
  pub(crate) fn build_and_attach_webview(
    ...
    // Sidebar browsers are created only after their DOM slot has stable bounds,
    // but keep the native child hidden until the first explicit layout command.
    // Doing this before returning the dispatcher prevents the inert bootstrap
    // document from flashing above the main webview.
    if child.label.starts_with("sidebar-browser-") {
  ...
  pub(crate) fn build_browser_child(
    ...
    // Child webviews render into their own native CEF surface. Offscreen
    // rendering remains available for explicit future experiments, but app
    // labels no longer opt into the CPU-frame path implicitly.
    let is_sidebar_browser = pending.label.starts_with("sidebar-browser-");
    let is_offscreen = pending.label.starts_with("offscreen-browser-");

    // If the bounds are not specified, default to the parent window's size and position.
    // aka full-window webview.
    let logical_bounds = pending.webview_attributes.bounds.unwrap_or_else(|| Rect {
    ...
    let bounds = logical_bounds.to_physical::<i32, i32>(scale);
    #[cfg(target_os = "macos")]
    let bounds = logical_bounds.to_logical::<i32, i32>(scale);
    let bounds = cef::Rect {
      x: bounds.position.x,
      y: bounds.position.y,
    ...
    let (init_done, on_initialized) = request_context::deferred_init_continuation({
      ...
      move |mut request_context| {
        ...
        // bounds_rate, so layout_app_window will not correct that bootstrap
        // position. Apply the requested rectangle once after BrowserHost exists
        // to prevent a newly recreated preview from covering the shell's top
        // left corner until the next DOM resize.
        #[cfg(windows)]
        let webview = {
          let mut webview = webview;
          webview.set_bounds(parent_size, scale, logical_bounds);
          webview
        };
