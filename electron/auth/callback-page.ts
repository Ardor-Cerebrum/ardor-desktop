const AUTH_FOCUS_PATH = "/auth/focus";

const CALLBACK_PAGE_TEMPLATE = `<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8">
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .handoff:hover { background: var(--action-hover); }
      .handoff:focus-visible { outline: 2px solid var(--action); outline-offset: 3px; }
...
export function renderAuthCallbackPage(status: number, message: string, focusToken?: string): string {
  const success = status >= 200 && status < 300;
  const handoff = success && focusToken
    ? `<form class="handoff-form" method="get" action="${AUTH_FOCUS_PATH}"><input type="hidden" name="token" value="${escapeHtml(focusToken)}"><button class="handoff" type="submit">Return to Ardor</button></form>`
    : "";

  return CALLBACK_PAGE_TEMPLATE
    .replace("%%DOCUMENT_TITLE%%", success ? "Return to Ardor" : "Sign-in issue — Ardor")
    .replace("%%STATE%%", success ? "success" : "error")
    .replace("%%TITLE%%", success ? "Sign-in received" : "Return to Ardor")
    .replace("%%MESSAGE%%", escapeHtml(message))
    .replace("%%HANDOFF%%", handoff);
}

export function renderAuthFocusPage(): string {
  return FOCUS_PAGE;
}
