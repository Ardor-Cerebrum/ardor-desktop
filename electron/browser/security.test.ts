...
describe("browser security policy", () => {
  ...
  test("upgrades explicit public HTTP navigation while preserving localhost and credentials", () => {
    expect(normalizeBrowserNavigationUrl("http://example.com/path?q=1")).toBe("https://example.com/path?q=1");
    expect(normalizeBrowserNavigationUrl("http://user:pass@example.com/path")).toBe(
      "https://user:pass@example.com/path",
    );
    expect(normalizeBrowserNavigationUrl("http://localhost:3000/path")).toBe("http://localhost:3000/path");
    expect(normalizeBrowserNavigationUrl("http://192.168.1.10/path")).toBeNull();
  });
