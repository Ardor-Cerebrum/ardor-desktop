...
test("virtualized CI has an explicit copy budget without weakening physical Macs", () => {
  ...
  assert.match(
    compositor,
    /ARDOR_TEST_METAL_COPY_P95_BUDGET_MS[\s\S]*Duration::from_millis\(500\)/,
  );
});

test("Metal acceptance uses a browser-free anchor and retries only startup transients", () => {
  const lib = readFileSync("src-tauri/src/lib.rs", "utf8");
  assert.match(lib, /context\.config_mut\(\)\.app\.windows\.clear\(\)/);
  assert.match(lib, /WindowBuilder::new\(app, "metal-test-anchor"\)/);
  assert.match(lib, /get_window\("metal-test-anchor"\)/);
  assert.match(testSupport, /while report\.completed_iterations < iterations/);
  assert.match(testSupport, /report\.startup_retries > 3/);
  assert.match(integrationTest, /report\.startup_retries <= 3/);
});
