...
The release is created as a draft before platform builds start. CI signs every updater artifact, generates both manifests, uploads the complete asset set, and only then publishes the release as `latest`. A failed build, metadata-signing step, or upload therefore cannot expose a partial update through the stable endpoints.

Manifests are generated by [scripts/generate-update-manifest.mjs](../scripts/generate-update-manifest.mjs) and reference the per-platform updater artifacts (`darwin-aarch64`, `windows-x86_64`). Generation is intentionally split into `prepare` and `finalize`: CI first creates canonical channel metadata, signs those exact bytes, and only then embeds the payload and its signature in `latest*.json`. The generator has a black-box test available through `bun run test:update-manifest`.
